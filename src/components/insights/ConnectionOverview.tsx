...
function Panel({
  ...
  return (
    <div
      className={`flex flex-col rounded-lg border border-border/50 bg-card/40 ${className}`}
    >
    ...
    <div className="space-y-1">
      ...
      <div className="h-1 overflow-hidden rounded-full bg-border/30">
        <div
          ...
          style={{
            width: `${Math.max(percent, value > 0 ? 12 : 0)}%`,
...
export function ConnectionOverview({
  ...
  const queryData = withFill(
    ...
    [...STORAGE_PALETTE].reverse(),
  );
  const hottestData = withFill(
    overview?.hottestTablesByParts ?? [],
    [...ENGINE_PALETTE].reverse(),
  );

  const pTotal =
    (overview?.activeQueryCount ?? 0) + (overview?.pendingMutationCount ?? 0);
  const qPct =
    pTotal > 0 ? ((overview?.activeQueryCount ?? 0) / pTotal) * 100 : 0;
  const mPct =
    pTotal > 0 ? ((overview?.pendingMutationCount ?? 0) / pTotal) * 100 : 0;

  const skel = (cls: string) => (
    <div className={`animate-pulse rounded-lg bg-muted/15 ${cls}`} />
  );
      ...
      {!overview && loading ? (
        <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-20 animate-pulse rounded-lg border border-border/40 bg-muted/10"
            />
          ))}
        </div>
      ) : null}
      ...
      {overview ? (
        <>
          ...
          <div className="grid grid-cols-2 gap-1.5 lg:grid-cols-4">
            <Stat
              label="Footprint"
              value={fmtBytes(overview.totalBytes)}
              sub={`${fmt(overview.totalRows)} rows`}
              icon={HardDrive}
              accent={C.cyan}
            />
            <Stat
              ...
              sub={`${overview.databaseCount} databases`}
              icon={Database}
              accent={C.teal}
            />
            <Stat
              ...
              sub={`${overview.pendingMutationCount} mutations`}
              icon={Activity}
              accent={C.amber}
            />
            <Stat
              label="Parts"
              value={fmt(overview.activePartCount)}
              sub="active parts"
              icon={Layers3}
              accent={C.violet}
            />
            ...
            {storageData.length > 0 ? (
              ready ? (
                <ChartContainer
                  ...
                  config={{ value: { label: "Bytes", color: C.cyan } }}
                >
                  <AreaChart
                    data={storageData}
              ...
              <div className="flex h-28 items-center justify-center text-[9px] text-muted-foreground/40">
                No storage data
              </div>
            )}
          </Panel>
              ...
              {engineData.length > 0 ? (
                ready ? (
                  <ChartContainer
                    ...
                    config={{ value: { label: "Tables", color: C.amber } }}
                  >
                    <BarChart
                      data={engineData}
                      layout="vertical"
                ...
                <div className="flex h-20 items-center justify-center text-[9px] text-muted-foreground/40">
                  No engine data
                </div>
              )}
            </Panel>
            ...
            <Panel title="Operational pressure" icon={Activity}>
              <div className="space-y-2.5">
                ...
                <PressureBar
                  ...
                  value={overview.pendingMutationCount}
                  percent={mPct}
                  color={C.amber}
                />
                <div className="grid grid-cols-2 gap-1.5 pt-0.5">
                  <div className="rounded-md border border-border/30 bg-background/30 px-2 py-1.5">
                    <div className="text-[8px] uppercase tracking-[0.16em] text-muted-foreground/40">
                      Server
                    </div>
                    <div className="mt-0.5 text-[11px] font-medium tabular-nums text-foreground">
                  ...
                  <div className="rounded-md border border-border/30 bg-background/30 px-2 py-1.5">
                    ...
                    <div className="mt-0.5 text-[11px] font-medium tabular-nums text-foreground">
                      {overview.databaseCount} DB · {overview.tableCount} tbl
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center gap-3">
                  {ready ? (
                    <ChartContainer
                      ...
                      config={{ value: { label: "Queries", color: C.teal } }}
                    >
                      <PieChart>
                        <Pie
                          data={queryData}
            ...
            <Panel title="Hottest tables" icon={Table2}>
              {hottestData.length > 0 ? (
                ready ? (
                  <ChartContainer
                    className="h-28 w-full"
                    config={{ value: { label: "Parts", color: C.coral } }}
                  >
                    <BarChart
                      data={hottestData}
                      layout="vertical"
                      margin={{ left: 2, right: 6, top: 0, bottom: 0 }}
                    >
                      <CartesianGrid
                        horizontal={false}
                        strokeDasharray="3 3"
