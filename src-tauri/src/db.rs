...
fn migrate_database(conn: &Connection) -> anyhow::Result<()> {
    ...
    if !has_column(conn, "connections", "ssh_tunnel_json")? {
        conn.execute("ALTER TABLE connections ADD COLUMN ssh_tunnel_json TEXT", [])?;
    }
    Ok(())
}
...
pub fn insert_audit_log(
    ...
    conn.execute(
        ...
        VALUES (?1, ?2, ?3, ?4, ?5)
        "#,
        params![connection_id, action, target, payload_json, Utc::now().to_rfc3339()],
    )?;
    Ok(())
}
...
pub fn insert_app_log(
    ...
    conn.execute(
        ...
        VALUES (?1, ?2, ?3, ?4, ?5)
        "#,
        params![level, category, message, context_json, Utc::now().to_rfc3339()],
    )?;
    Ok(())
}
...
pub fn backup_database(path: &Path, target_path: &Path) -> anyhow::Result<()> {
    ...
    let conn = open_db(path)?;
    conn.execute("VACUUM INTO ?1", [target_path.to_string_lossy().to_string()])?;
    Ok(())
}
