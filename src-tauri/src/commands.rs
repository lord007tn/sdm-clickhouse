...
use tauri::{async_runtime::spawn_blocking, Emitter, State};
use uuid::Uuid;
...
fn read_secret(state: &AppState, connection_id: &str) -> anyhow::Result<String> {
    ...
    Err(anyhow::anyhow!(
        "missing secret for connection {}",
        connection_id
    ))
}

fn write_secret(state: &AppState, connection_id: &str, password: &str) -> anyhow::Result<()> {
...
fn delete_secret(state: &AppState, connection_id: &str) {
...
fn github_token() -> Option<String> {
    // Only use a token explicitly set for the updater. The default repo is
    // public so no auth is needed. GITHUB_TOKEN / GH_TOKEN are intentionally
    // ignored because CI-scoped or CLI-scoped tokens leak into the desktop
    // environment and cause 403 errors against the public releases API.
    std::env::var("SDM_CLICKHOUSE_UPDATER_TOKEN")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}
...
async fn fetch_releases(repo: &str) -> Result<Vec<GithubRelease>, String> {
    ...
    if !status.is_success() {
        let guidance = if status.as_u16() == 404 {
            ...
            "Authentication failed for GitHub API. Check SDM_CLICKHOUSE_UPDATER_TOKEN if set."
                .to_string()
        } else {
...
fn select_asset_for_target<'a>(
    ...
    if os == "linux" {
        if prefer_portable {
            if arch == "arm64" {
                if let Some(asset) =
                    find(|name| name.ends_with(".appimage") && name.contains("aarch64"))
                {
                    return Some(asset);
                }
                if let Some(asset) =
                    find(|name| name.ends_with(".appimage") && name.contains("arm64"))
                {
                    return Some(asset);
                }
            } else {
                if let Some(asset) =
                    find(|name| name.ends_with(".appimage") && name.contains("amd64"))
                {
                    return Some(asset);
                }
                if let Some(asset) =
                    find(|name| name.ends_with(".appimage") && name.contains("x64"))
                {
                    return Some(asset);
                }
            }
    ...
    if os == "macos" {
        if prefer_portable {
            if arch == "arm64" {
                ...
                name.ends_with(".app.tar.gz") && (name.contains("x64") || name.contains("amd64"))
            }) {
                return Some(asset);
            }
...
#[tauri::command]
pub async fn connection_list(state: State<'_, AppState>) -> Result<Vec<ConnectionProfile>, String> {
    let app_state = state.inner().clone();
    spawn_blocking(move || db::list_connections(&app_state.db_path).map_err(into_message))
        .await
        .map_err(|err| err.to_string())?
}
...
pub async fn connection_delete(
    ...
    let app_state = state.inner().clone();
    spawn_blocking(move || {
        db::delete_connection(&app_state.db_path, &connection_id).map_err(into_message)?;
        delete_secret(&app_state, &connection_id);
        Ok(CommandMessage {
...
pub async fn connection_save(
    ...
    spawn_blocking(move || {
        ...
        let profile =
            db::upsert_connection(&app_state.db_path, &normalized).map_err(into_message)?;
        if has_password {
            write_secret(&app_state, &profile.id, &password).map_err(into_message)?;
        }
        Ok(profile)
    })
    .await
...
pub async fn connection_test(
    ...
    let (profile, password) = spawn_blocking(move || {
        let profile = build_connection_profile_from_input(&payload_for_blocking, "ad-hoc", 10_000)
            .map_err(into_message)?;
        let password =
            resolve_connection_password(&app_state, &payload_for_blocking).map_err(into_message)?;
        Ok::<_, String>((profile, password))
    })
    .await
    .map_err(|err| err.to_string())??;
...
pub async fn query_execute(
    ...
    let result = match kind {
        QueryKind::Select => {
            ...
            execute_sql_json(&profile, &password, &query_sql, timeout_ms, Some(&query_id))
                .await
...
pub async fn connection_diagnostics(
    ...
    let (profile, password) = spawn_blocking(move || {
        let profile =
            build_connection_profile_from_input(&payload_for_blocking, "diagnostics", 10_000)
                .map_err(into_message)?;
        let password =
            resolve_connection_password(&app_state, &payload_for_blocking).map_err(into_message)?;
        Ok::<_, String>((profile, password))
...
pub fn connection_import_profiles(
    ...
    let imported =
        db::import_connections(&state.db_path, &parsed, overwrite_existing.unwrap_or(false))
            .map_err(into_message)?;
    Ok(CommandMessage {
...
fn launch_installer(installer_path: &Path, asset_name: &str) -> anyhow::Result<()> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if ext == "msi" {
            ...
            Command::new(installer_path).arg("/CURRENTUSER").spawn()?;
        }
        Ok(())
    }
